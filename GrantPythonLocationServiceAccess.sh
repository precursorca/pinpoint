...
#	- v.1.8: 2025.02.09 Alex Narvey: Added code for macOS 14 and macOS 15, Customized App for Python.
#	- v.1.8.1: 2025.08.20 Alex Narvey: Added code for macOS 26 Tahoe.
####################################################################################################
# This version modified to instead default to granting Python permission
scriptVersion="2025.08.20"
...
clients="/var/db/locationd/clients.plist"
    ...
    updateScriptLog "Executing for macOS 26 Tahoe..."
    clients="/var/db/locationd/clients.plist"
        if [[ -f "$clients" ]]; then
            updateScriptLog "$clients already exists! Moving on..."
